  ...
  button_add.on('click', function(event) {
    ...
    frame.on('select', function() {
      ...
      var attachment_thumbs = selection.map(function(attachment) {
        attachment = attachment.toJSON();
        if (attachment.id != '') {
          var src = (attachment.sizes.thumbnail.length)
                  ?  attachment.sizes.thumbnail.url
                  :  attachment.sizes.full.url
          return '<label><input type="checkbox" name="realpostimages_images[' + attachment.id + ']" value="' + encodeURIComponent(JSON.stringify(attachment.sizes)) + '" class="hidden" checked="checked" data-id="' + attachment.id + '" /><img src="' + src + '" /></label>';
        }
      }).join(' ');
